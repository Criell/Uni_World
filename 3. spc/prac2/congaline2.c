...
{	
	...
	for (j = 0; j < i; j++){
        ...
        if ( pid == 0) {
            ...
            // make strings with the program and 
            // a char array with the arguments

            program = strtok (line[j], " ");
            strcpy(argvs[0], program);  
            int n = 1;
            while( (program = strtok (NULL, " ")) != NULL ){
                strcpy(argvs[n], program);
                n++;
            }

            //copy the char array into a char*[]
            //with the propose of use execvp
            char *argv[n+1]; 
            int k;
            for ( k = 0; k < n; k++){
                argv[k] = argvs[k];
